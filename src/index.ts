    ...
    this.server.tool(
      ...
      async (params) => {
        try {
          ...
          const availableAtoms = atomicComponentsData.components.map((c: any) => c.name);
          ...
          const atomDetails = params.atomicComponents.map(atomName => {
            const atomComponent = atomicComponentsData.components.find((c: any) => c.name === atomName);
            if (!atomComponent) return { name: atomName, note: "Component details not found" };
          ...
          if (!params.atomicComponents.includes('BaseWidgetComponent')) {
            const baseWidgetComp = atomicComponentsData.components.find((c: any) => c.name === 'BaseWidgetComponent');
            if (baseWidgetComp) {
          ...
          const componentUsageMap: Record<string, string[]> = {
            "buttons": ["Button"],
            "text": ["Typography"],
          ...
          let detailedAtomicComponentsData: Record<string, {
            description: string;
          ...
          try {
            ...
            if (fs.existsSync(atomicComponentsPath)) {
              ...
              atomicComponents.forEach((componentName: string) => {
                const componentInfo = atomicComponentsJson.components.find((c: any) => c.name === componentName);
                if (componentInfo) {
          ...
          // These properties may not exist in the params type but are used in the code
          // Use type assertion with optional properties
          interface ExtendedParams {
            clientFramework?: 'react' | 'next' | 'gatsby';
          ...
          const fileTemplates: Record<string, {
            fileName: string;
            content: string;
            description?: string;
            path: string;
            ...
            content: "import React from 'react';\n" +
                     ...
                     (atomicComponents.length > 0 
                      ? atomicComponents
                          ...
                          .map((comp: string) => 
                            `import { ${comp} } from '../../atomic-components/${comp}';\n`
                          ).join("")
                      : "") +
                     "\n" +
            ...
            clientInstructions: {
              ...
              atomicComponents: atomicComponents.filter(comp => validAtomicComponents.includes(comp)).length > 0 ? [
                ...
                ...atomicComponents
                  .filter(comp => validAtomicComponents.includes(comp))
                  .map((comp: string) => 
                    `- ${comp}: ${detailedAtomicComponentsData[comp]?.description || "No description available"}`
                  )
              ] : [
    ...
    this.server.tool(
      ...
      async ({ widgetName, basePath, clientFramework = 'react', missingFiles = [], atomicComponents = [], fileTemplates, runClientCommands = false }) => {
        try {
          ...
          let actualMissingFiles: string[] = [...missingFiles];
          ...
          let missingFileTemplates: Record<string, any> = {};
